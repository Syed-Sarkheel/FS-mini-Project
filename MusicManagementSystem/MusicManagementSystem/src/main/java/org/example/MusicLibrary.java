...
class MusicLibrary {
    private Map<String, Long> indexMap;//An object that maps keys to values. A map cannot contain duplicate keys; each key can map to at most one value.
    ...
    public void addSong(Song song) throws IOException {
        if (!(indexMap.containsKey(song.getTitle()) && indexMap.containsKey(song.getArtist()))) {
